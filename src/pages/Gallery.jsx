...
import { motion, useInView, useScroll, useTransform } from "framer-motion";
import "./Gallery.css";
// The import for 'debug' from 'three/src/nodes/TSL.js' seems unused and can be removed.

const Gallery = () => {
    // --- CODE FOR AUTOMATIC REFRESH ---
    useEffect(() => {
        // We use sessionStorage to track if a refresh has already happened.
        // This prevents an infinite loop of refreshes.
        const hasRefreshed = sessionStorage.getItem("galleryRefreshed");

        if (!hasRefreshed) {
            // If the flag is not set, set it now.
            sessionStorage.setItem("galleryRefreshed", "true");
            // Then, reload the page.
            window.location.reload();
        }

        // This effect should only run once when the component mounts.
        // The cleanup function removes the flag when the user navigates away.
        return () => {
            sessionStorage.removeItem("galleryRefreshed");
        };
    }, []); // The empty dependency array ensures this runs only once on mount.

    useEffect(() => {
        // This effect for Shery.js will run after the page reloads.
        window.Shery.imageEffect(".diagonal-image-container", {
            style: 4,
        });
    }, []);
